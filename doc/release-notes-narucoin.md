Narucoin Core version *0.15.1* is now available from:

  <https://download.narucoin.org/narucoin-0.15.1/>
...
A number of changes to the way Narucoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 sasus per kB (0.00001 NARC). This is to prepare for dropping the minimum transaction fee to 0.1 sasus per kB (0.0001 NARC) in the next release.
