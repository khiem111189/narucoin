...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 41432
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 21028
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 61432
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 121028
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 41432
	%{_sbindir}/semanage port -d -p tcp 21028
	%{_sbindir}/semanage port -d -p tcp 61432
	%{_sbindir}/semanage port -d -p tcp 121028
	for selinuxvariant in %{selinux_variants}; do
